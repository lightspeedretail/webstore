...
//		);
		...
		if ($objCustomer instanceof Customer) {
			if(isset($objCustomer->defaultShipping))
			{
				...
				if($objDestination) {
					...
					if ($objDestination->taxcode0->IsNoTax() && _xls_get_conf('TAX_INCLUSIVE_PRICING') && _xls_get_conf('SHOW_TAX_SWITCH_WARNING',1))
	...
	public function AddProduct($objProduct,
		$intQuantity = 1, $mixCartType = false, $intGiftItemId = null,$strDescription = false,$fltSell = false,$fltDiscount = false) {
	...
	public static function AddToCart($objProduct,
		...
		//Need to pass along $fltSell etc so they get used if defined
		return $objCart->AddProduct($objProduct, $intQty, $mixCartType, $intGiftItemId, $strDescription,$fltSell,$fltDiscount );
