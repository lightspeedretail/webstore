...
class ShoppingCart extends CApplicationComponent
{
			...
			if (count($arrPastItems)>0)
			{
				foreach($arrPastItems as $objItem) {
					...
					$this->model->AddToCart($objProduct,$objItem->qty,$objItem->description,
						$objItem->sell,0,$objItem->cart_type,$objItem->wishlist_item); //we strip any discount from another cart usu. promo code
	...
	public function loginQuote($objDocument= null)
	{
		...
		if (count($arrPastItems)>0) {
			foreach($arrPastItems as $objItem) {
				$objProduct = Product::model()->findbyPk($objItem->product_id);
				$retVal = $this->model->AddToCart($objProduct,$objItem->qty,$objItem->description,
					$objItem->sell,$objItem->discount,CartType::order,$objItem->gift_registry_item);
				if (strlen($retVal)>5)
					return $retVal;
