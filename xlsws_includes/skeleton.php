 ...
 */
	...
	class xlsws_index extends QForm{
		...
		public function continue_shopping($strFormId, $strControlId, $strParameter){
			...
			QQ::AndCondition(
				...
				, QQ::In(QQN::ViewLog()->LogTypeId, array(ViewLogType::categoryview , ViewLogType::productview , ViewLogType::search , ViewLogType::pageview))
		...
		public function search_guesses($strFormId, $strControlId, $strParameter){
			...
			Product::QueryArray( 
							...
							QQ::OrCondition(
     	 					...
     	 					, QQ::Clause(
     	 					    QQ::OrderBy(
    	 						  $this->generateLikeSearchNodeForProduct(QQN::Product()->Code , "'$search%'") 
    	 						, false
    	 						, $this->generateLikeSearchNodeForProduct(QQN::Product()->Name , "'$search%'") 
    	 						, false
    	 						, $this->generateLikeSearchNodeForProduct(QQN::Product()->Code , "'%$search%'") 
    	 						, false
    	 						, $this->generateLikeSearchNodeForProduct(QQN::Product()->Name , "'%$search%'") 
         ...
         * @return none
         */ 				
        protected function update_order_display($objCart,
		...
		protected function create_prod_img($parent , $prod , $imgType , $width , $height , $ajax_add_action = 'add_to_cart' , $action_parameter = false){
	        ...
	        $this->arrProdDragImages[$prod->Rowid] = array('panel' => $pnlImg 
	        											, 'prod_id' => $prod->Rowid 
	        											, 'id' => $pnlImg->ControlId
	        											, 'parent' => $parent 
	...
	public static function send_email($cart)
	{
		...
		_xls_mail(_xls_get_conf('ORDER_FROM')
		, _xls_get_conf('STORE_NAME' , 'Web') . " " . _sp("Order Notification") . " " . $order_id
