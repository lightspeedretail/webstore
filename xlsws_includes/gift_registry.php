...
class xlsws_gregistry extends xlsws_index {
		...
		protected function btnRecMail_Click($strFormId, $strControlId, $strParameter) {
			...
			_xls_mail(_xls_mail_name($objGiftRec->ReceipentName , $objGiftRec->ReceipentEmail) , "Wish List in " . _xls_get_conf('STORE_NAME') , _xls_mail_body_from_template(templatenamed('email_gift_registry.tpl.php') , array('receipent' =>$objGiftRec, 'gift' => $objGift )));
		...
		protected function pxyMailAll_Click($strFormId, $strControlId, $strParameter) {
			...
			foreach($objGiftRec as $objRec){
				...
				_xls_mail(_xls_mail_name($objRec->ReceipentName , $objRec->ReceipentEmail) , "Wish List in " . _xls_get_conf('STORE_NAME') , _xls_mail_body_from_template(templatenamed('email_gift_registry.tpl.php') , array('receipent' =>$objRec, 'gift' => $objGift )));
			}
