   ...
   class xlsws_fpassword extends xlsws_index {
		...
		public function getPassword($strFormId, $strControlId, $strParameter){
			...
			if($cust && $cust->AllowLogin==0){
				_xls_display_msg(_sp("Your login is not active yet. ") . _sp("Please ") . "<a href=\"index.php?xlspg=contact_us\">" . _sp("contact us") . "</a>" . _sp(" if you need urgent assistance."));

			}elseif($cust){
				
				_xls_mail(_xls_mail_name($cust->Mainname , $cust->Email) , _sp("Password reminder") , _xls_mail_body_from_template(templatenamed('email_forgot_password.tpl.php') , array('cust' =>$cust)));
