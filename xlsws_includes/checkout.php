...
class xlsws_checkout extends xlsws_index {
	...
	{
		if($this->customer){
			$cart_addes = Cart::QueryArray(QQ::AndCondition( QQ::Equal(QQN::Cart()->CustomerId , $this->customer->Rowid) , QQ::Equal(QQN::Cart()->Type , CartType::order) ) , QQ::Clause(QQ::OrderBy(QQN::Cart()->Rowid , false)));
			...
			if($cart_addes && (count($cart_addes) > 0)){
				...
				foreach($cart_addes as $adde){
					$a = sprintf("%s %s %s %s, %s" , $adde->ShipFirstname , $adde->ShipLastname , $adde->ShipCompany , $adde->ShipAddress1 , $adde->ShipCity);
	...
	public function setupShipping(){
		...
		$shippingModules = Modules::QueryArray(QQ::Equal(QQN::Modules()->Type, 'shipping' )
	...
	protected function shippingCost(){
		...
		// Cost for shipping?
		$total = $obj->total($this->shipping_fields
		, $cart
		, $this->txtCRShipCountry->SelectedValue
		, $this->txtCRShipZip->Text
		, $this->txtCRShipState->SelectedValue
		...
		if($total === FALSE){ // no shipping available..
			...
			$this->fltShippingCost = FALSE;
			$this->lblShippingCost->Text = _sp('Error: Unable to get shipping rates');
			_xls_log("ERROR: Could not determine return type for module " . $this->lstShippingMethod->SelectedValue . ". Expected either number OR array('price' => ,  'msg' =>). Return given : " . print_r($total , true));
		}
	...
	public function setupPayment($remove = false){
		...
		$paymentModules = Modules::QueryArray(QQ::Equal(QQN::Modules()->Type, 'payment' )
		, QQ::Clause(QQ::OrderBy(QQN::Modules()->SortOrder)));
	...
	protected function Form_Validate() {
		...
		if($this->txtCREmail->Text == "" || $this->txtCRMPhone->Text == "" || $this->txtCRFName->Text == "" || $this->txtCRLName->Text == "" || $this->txtCRBillAddr1->Text == "" || $this->txtCRBillCountry->SelectedValue == "" || $this->txtCRBillCity->Text == "" || $this->txtCRBillZip->Text == "" ){
		...
		if(_xls_get_conf('SHIP_RESTRICT_DESTINATION' , 0) == 1){
			...
			if(!$dest){
				...
				$errors[] =  sprintf(_sp("Sorry, we cannot ship to %s %s %s") , $this->txtCRShipState->SelectedName , $this->txtCRShipZip->Text , $this->txtCRShipCountry->SelectedName);
	...
	public function processCheckout($strFormId, $strControlId, $strParameter){
		...
		while( $tmp = _dbx_first_cell("SELECT rowid FROM xlsws_cart WHERE id_str = '$id' AND rowid <> '$cart->Rowid'")){ // if there is a order allocated in this id already!
		...
		$cart->AddressBill = $this->txtCRBillAddr1->Text
		. "\n" . $this->txtCRBillAddr2->Text
		. "\n" . $this->txtCRBillCity->Text
		. "\n" . $this->txtCRBillState->SelectedValue . " " . $this->txtCRBillZip->Text   // TODO for countries that doesn't have state?
		...
		$cart->AddressShip = $cart->ShipFirstname . " " . $cart->ShipLastname
		. (($cart->ShipCompany != '')?("\n" . $cart->ShipCompany ):"")
		. "\n" . $cart->ShipAddress1
		. "\n" . $cart->ShipAddress2
		. "\n" . $cart->ShipCity
