...
while ($objItem = $arrProducts->FetchObject()) {
  ...
  echo '<item>'.chr(13);
      echo chr(9)."<g:id>".$objProduct->Rowid."</g:id>".chr(13);
       echo chr(9).'<title>'.strip_tags($objProduct->Name).'</title>'.chr(13);
	   ...
	   echo chr(9).'<g:product_type><![CDATA['.implode(" &gt; ",Category::GetTrailByProductId($objProduct->Rowid,'names')).']]></g:product_type>'.chr(13);
       echo chr(9).'<link>'.$objProduct->Link.'</link>'.chr(13);
	   echo chr(9).'<g:image_link>http://www.foryarnssake.com/store/'.$objProduct->Image.'</g:image_link>'.chr(13);
