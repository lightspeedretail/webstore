...
class fedex extends xlsws_class_shipping {
	...
	public function total($fields , Cart $cart , $country = '' , $zipcode  = '' , $state = '', 
		...
		$request['WebAuthenticationDetail'] = 
			array('UserCredential' => array('Key' => $config['authkey'], 'Password' => $config['securitycode'])); 
		...
		if (($length + $width + $height) == 0){
			...
			array('0' => array('SequenceNumber' => '1',
                  'InsuredValue' => array('Amount' => $cart->Subtotal,
                                          'Currency' => _xls_get_conf('CURRENCY_DEFAULT' , 'USD')
                                          ),
                  'ItemDescription' => 'Ordered items',
                  'Weight' => array('Value' => $weight, 'Units' => 'LB'),
