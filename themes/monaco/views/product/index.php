		...
		<div class="row-fluid">
	        ...
	        <div class="span7">
		        ...
		        <?php if (_xls_get_conf('USE_SHORT_DESC'))
			        echo CHtml::tag('div',
				        array('id'=>CHtml::activeId($model,'description_short'),'class'=>'row description', 'style'=>$model->WebShortDescription ? '' : 'display: none;'),
			            $model->WebShortDescription);
			    ?>

                <div class="row">
                    <?= CHtml::tag('div',
                        array('id'=>CHtml::activeId($model,'description_long'),'class'=>'', 'style'=>$model->WebLongDescription ? '' : 'display: none;'),
					            ...
					            'success' => 'js:function(data){
				                    if (data.action=="alert") {
				                      alert(data.errormsg);
										...
										'.(_xls_get_conf('AFTER_ADD_CART') ? 'window.location.href="'.$this->createUrl("/cart").'"' : '$("#shoppingcart").html(data.shoppingcart);').'
