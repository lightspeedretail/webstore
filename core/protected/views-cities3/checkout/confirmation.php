<div class="section-content" id="confirm">
	...
	<div class="final-confirmation">
		...
		<div class="comments">
			<a href="#" onclick="$('.comments a').hide(); $('.comments textarea').fadeToggle().focus(); return false;">
				<?php echo Yii::t('checkout', 'Add a comment or special request'); ?>
			</a>
			<?php
			echo $form->textArea(
				$model,
				...
				<div style="position:relative;">
					...
					echo CHtml::textField(
						...
						array(
							...
							'onkeypress' => 'return confirmation.ajaxTogglePromoCodeEnterKey(event, ' .
					...
					echo CHtml::htmlButton (
						...
						array(
							...
							'onclick' => 'confirmation.ajaxTogglePromoCode(' .
								json_encode(CHtml::activeId('Confirmation','promoCode')) .
								');'
						)
					);
                    ...
                    $.ajax({url: '".Yii::app()->controller->createUrl('cart/updatecart')."',
                            ...
                            success: function(data) {
                                if(data.action=='success') {
                                    confirmation.redrawCart(JSON.parse(data.cartitems));
                                }
	                            else if(data.errorId === 'invalidQuantity'){
                                    var qty = $('#' + pk);
                                    qty.val(data.availQty);
                                    confirmation.tooltip.createTooltip(pk, '<strong>Only ' + data.availQty + ' are available at this time.</strong><br> If you&quot;d like to order more please return at a later time or contact us.');
                                }
                                else {
                                    alert(data.errormsg);
                                }
