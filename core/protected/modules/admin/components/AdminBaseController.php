...
class AdminBaseController extends CController
{
	...
	public function verifySSL()
	{
		...
		if ($this->getId() != 'license' &&
			Yii::app()->params['ENABLE_SSL'] &&
			!Yii::app()->user->getState('internal', false))
		{
	...
	public function actionModule()
	{
		...
		if (!is_null($model))
		{
			// Get form elements (Admin panel configuration) and add our layout
			// formatting so the form looks nice within Admin Panel.
			$this->editSectionInstructions = $this->getInstructions(get_class($this)) .
				'<p>' . $this->editSectionInstructions;
			...
			if(isset($_POST[$adminModelName]))
			{
				...
				if ($model->validate())
				{
					...
					$objModule->SaveConfigValues($newConfig);
					...
					else
					{
						...
						Yii::app()->getComponent($id)->init();

						// If we happen to be updating a module that includes a
						// promo code, we need to throw that to our restrictions.
						...
						if($id == 'wstheme')
						{
							$strNewChild = $newConfig['CHILD_THEME'];
							if ($strOldChild !== $strNewChild && $strNewChild !== 'custom')
							{
								Yii::app()->theme->config->activecss =
									$this->updateActiveCss($strNewChild, $strOldChild);
							}
						}
					}
				}
				else
				{
					...
					else
					{
						Yii::app()->user->setFlash(
							'error',
							Yii::t('global', 'Error saving, check form fields for specific errors')
						);
					}
				}
			}
		...
		if (!is_null($model))
		{
			...
			if(isset($_POST[$adminForm]))
			{
				...
				$this->registerOnOff(
					$objModule->id,
					'Modules_active',
					_xls_number_only($_POST['Modules']['active'])
				);
...
SETUP;
	...
	public function scanModules($moduletype = "payment")
	{
		...
		{
			$files = glob(
				YiiBase::getPathOfAlias('webroot.themes') . '/*',
				GLOB_ONLYDIR
			);
		...
		else
		{
			...
			$files = array_merge(
				glob(
					realpath(YiiBase::getPathOfAlias('ext.ws'.$moduletype)) . '/*',
					GLOB_ONLYDIR
				),
				$arrCustom
			);
		...
		foreach ($files as $file)
		{
			...
			if(!($objModule instanceof Modules))
			{
				...
				try {
					...
					{
						Yii::log(
							"Found widget $moduleName could not install " . print_r($objModule->getErrors(), true),
							'error',
					...
					Yii::log(
						"Found $moduletype widget $moduleName could not install ".$e,
						'error',
						'application.'.__CLASS__.".".__FUNCTION__
					);
				}
	...
	/**
	 * Boolean if this store is a Multitenant Store (could be Cloud or Pro).
	 *
	 * @return boolean
	 */
	...
	/**
	 * Boolean if this store is a Hosted store.
	 *
	 * @return boolean
	 */
