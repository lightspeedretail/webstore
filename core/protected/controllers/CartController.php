...
class CartController extends Controller
{
	...
	public function actionReceipt()
	{
		...
		if (Yii::app()->theme->info->advancedCheckout)
		{
			...
			$this->redirect($this->createAbsoluteUrl("/checkout/thankyou",array('linkid' => $strLink )));
		}
...
//					$objEmail->plainbody = $strPlain;
	...
	{
		// country_id is actually a country code, but we can't change the view
		// because of the possibility of customizations.
		$strCountryCode = Yii::app()->getRequest()->getPost('country_id');
		$intCountryId = Country::IdByCode($strCountryCode);

		if (empty($intCountryId) === true)
		{
	...
	public function actionApplyPromocode()
	{
		...
		echo json_encode($this->_applyPromoCode($strPromoCode));
	}
	...
	public function actionModalApplyPromocode()
	{
		...
		$result = $this->_applyPromoCode($strPromoCode, false);
	 ...
	 * @param Boolean $renderHtml Whether to return rendered HTML in the result.
	 * @return Array An Array containing the result of the action.
	 */
	private function _applyPromoCode($strPromoCode, $renderHtml = true)
	{
		$objCart = Yii::app()->shoppingcart;
		if ($objCart->fk_promo_id > 0)
		{
		...
		if ($objPromoCode->Shipping)
		{
			...
			if (Yii::app()->theme->info->advancedCheckout === false)
			{
				...
				if ($checkoutForm !== null)
				{
					...
					{
						$arrFormattedCartScenario = self::formatCartScenarios($arrCartScenario);
						$arrReturn['cartItems'] = $arrFormattedCartScenario['renderedCartItems'];
	...
	protected function formatCartScenarios($arrCartScenario)
	{
		...
		{
			$shippingModuleId = $cartScenario['providerId'];
			...
			$arrShippingOptionPrice[$shippingModuleId][$priorityIndex] = $cartScenario['formattedShippingPrice'];

			// Formatted shipping prices for each scenario.
			$arrFormattedShippingOptionPrice[$shippingModuleId][$priorityIndex] =
				$cartScenario['formattedShippingPrice'];

			// Cart totals for each scenario.
			$arrFormattedCartTotal[$shippingModuleId][$priorityIndex] =
				$cartScenario['formattedCartTotal'];
			...
			// theme customization.
			$savedShoppingCart = Yii::app()->shoppingcart->getModel();
			$cart = new Cart();
			$cart->attributes = $cartScenario['shoppingCart'];
			Yii::app()->shoppingcart->setModel($cart);
			$arrRenderedCartItems[$shippingModuleId] = str_replace(
				"\n",
				'',
				$this->renderPartial('/cart/_cartitems',null,true)
			);

			// The rendered _carttaxes partials.
			$arrRenderedCartTaxes[$shippingModuleId][$priorityIndex] = $this->renderPartial(
				'/cart/_carttaxes',
				array('model' => Yii::app()->shoppingcart),
				true
			);

			Yii::app()->shoppingcart->setModel($savedShoppingCart);
	...
	public function actionAjaxCalculateShipping()
	{
		...
		$arrReturn = array(
			...
			'prices' => $arrFormattedCartScenario['shippingOptionPrices'],
