...
class destinationshipping extends WsShipping
{
	...
	public function run() {
		...
		// Without all of the values we can't do a lookup.
		if (empty($country) ||
			empty($state) ||
			empty($zipcode))
		{
			Yii::log('DESTINATION TABLE: insufficient details for destination shipping', 'info', 'application.'.__CLASS__.'.'.__FUNCTION__);
			return false;
		}
