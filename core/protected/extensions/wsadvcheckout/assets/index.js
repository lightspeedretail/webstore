$(function () {
    ...
    login.validateForm = function(e) {
        ...
        if (!$('input[type="email"]')[0].validity.valid) {
            ...
            !$('footer .login').hasClass('invisible')
                && loginButton.hasClass("is-login")
                && (!$('input[type="password"]')[0].validity.valid || $($('input[type="password"]')[0]).val() == "")
            ) {
            login.renderError(form, login.PASSWORD_REQUIRED);
            $("#LoginForm_password").focus();
            return false;
        }
        return true;
    ...
    $('.ordered_before').on('click', function (e) {
        ...
        // Change title
        login.formTitleText('Login to your account')
    ...
    $('input.new_customer').on('click', function (e) {
        ...
        else if ($('footer .login').hasClass('invisible')) {
            // Change to continue button
            login.changeFooter('new_customer');
            login.clearErrors(form);
        }
        return false;
    });
    ...
    $('.guest_checkout').on('click', function (e) {
        ...
        login.formTitleText('Guest Checkout')
